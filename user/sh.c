...
// History buffer
char history[HISTORY_SIZE][MAXBUF];
int history_count = 0;

// xv6-compatible string helpers
void xstrncpy(char *dst, char *src, int n){
  int i;
...
int main(void){
  ...
  while(getcmd(buf,sizeof(buf))>=0){
    ...
    xstrncpy(history[history_count % HISTORY_SIZE], buf, MAXBUF);
    history_count++;

    // Builtin commands
    if(xstrncmp(cmdline,"cd ",3)==0){
      cmdline[strlen(cmdline)-1]=0;
      if(chdir(cmdline+3)<0) fprintf(2,"cannot cd %s\n",cmdline+3);
      continue;
    }
    if(xstrncmp(cmdline,"history",7)==0){
    ...
    if(fork1()==0) runcmd(parsecmd(cmdline));
    wait(0);
  }
  exit(0);
}
...
// ======================
// Parsing (no changes from xv6 original)
// ======================
char whitespace[]=" \t\r\n\v";
char symbols[]="<|>&;()";
...
int gettoken(char **ps,char *es,char **q,char **eq){
  ...
  switch(*s){
    ...
    default: ret='a'; while(s<es && !strchr(whitespace,*s) && !strchr(symbols,*s)) s++; break;
  }
  if(eq) *eq=s;
  while(s<es && strchr(whitespace,*s)) s++;
  *ps=s; return ret;
...
struct cmd* parseexec(char **ps,char *es){
  char *q,*eq; int tok,argc; struct execcmd *cmd; struct cmd *ret;
  if(peek(ps,es,"(")) return parseblock(ps,es);
  ret=execcmd(); cmd=(struct execcmd*)ret; argc=0; ret=parseredirs(ret,ps,es);
  while(!peek(ps,es,"|)&;")){
    if((tok=gettoken(ps,es,&q,&eq))==0) break;
    if(tok!='a') panic("syntax");
    cmd->argv[argc]=q; cmd->eargv[argc]=eq; argc++; if(argc>=MAXARGS) panic("too many args");
...
struct cmd* nulterminate(struct cmd *cmd){
  int i; struct backcmd *bcmd; struct execcmd *ecmd; struct listcmd *lcmd; struct pipecmd *pcmd; struct redircmd *rcmd;
  if(cmd==0) return 0;
  switch(cmd->type){
    case EXEC: ecmd=(struct execcmd*)cmd; for(i=0;ecmd->argv[i];i++) *ecmd->eargv[i]=0; break;
    case REDIR: rcmd=(struct redircmd*)cmd; nulterminate(rcmd->cmd); *rcmd->efile=0; break;
    case PIPE: pcmd=(struct pipecmd*)cmd; nulterminate(pcmd->left); nulterminate(pcmd->right); break;
    case LIST: lcmd=(struct listcmd*)cmd; nulterminate(lcmd->left); nulterminate(lcmd->right); break;
    case BACK: bcmd=(struct backcmd*)cmd; nulterminate(bcmd->cmd); break;
  }
  return cmd;
}
